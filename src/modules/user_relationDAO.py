...
class UserRelationDAO:
	...
	def update(self,new_data:tuple):
		...
		self.conn.execute(query,query_data=data)
		result = self.conn.query("SELECT * FROM relation_type WHERE first_user = %s AND second_user = %s AND relation_type = %s",tuple([data[1],data[2],data[0]]))
		self.conn.commit()
		return len(result)

	def delete(self,users:tuple):
		query = "DELETE FROM relation_type WHERE first_user = %s AND second_user = %s"
	...
	def select_relation_of_users(self,users):
		query = "SELECT relation_type FROM relation_type WHERE first_user = %s AND second_user = %s"
		data = tuple(sorted([users[0],users[1]]))
		return self.conn.query(query,query_data=data)
